...
import { COURSE_COMPONENTS } from "@/components/courses";
...
const Article = () => {
  ...
  // Show course modal state (already declared at the top, so remove this duplicate)
  // const [showCourse, setShowCourse] = useState(false);

  // Generate a slug for the article to match with course components
  const slug = article.title
    ? article.title
        ...
        .replace(/(^-|-$)/g, "")
    : "";
  const CourseComponent = COURSE_COMPONENTS[slug];
    ...
    <>
      ...
      <article className='prose prose-lg max-w-none'>
        <div className='mb-6 flex flex-wrap items-center gap-3'>
          <Badge
            variant='outline'
            className='text-primary-600 bg-primary-50/50'
          >
            {categoryLabel}
          </Badge>
          <h1 className='mt-0 !mb-0 text-3xl font-bold text-neutral-800'>
            {article.title}
          </h1>
        </div>
      ...
      {/* Modal for the course */}
      <CourseModal open={showCourse} onClose={() => setShowCourse(false)}>
        {typeof CourseComponent === "function" ? <CourseComponent /> : null}
      </CourseModal>
          ...
          <div className='grid gap-6 sm:grid-cols-2 lg:grid-cols-3'>
            {relatedArticles.map((relatedArticle) => (
              <Card
                ...
                className='group overflow-hidden hover:border-primary-200'
              >
      ...
      {/* Gamified course button for articles with a course */}
      {typeof CourseComponent === "function" && (
        <div className='my-6'>
          <Button
            onClick={() => setShowCourse(true)}
            className='bg-green-600 hover:bg-green-700 text-white'
          >
            Start Gamified Course
          </Button>
        </div>
      )}
    </>
