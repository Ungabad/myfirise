...
export interface IStorage {
  ...
  getBudgetByCategory(userId: number, categoryId: number, month: number, year: number): Promise<Budget | undefined>;
...
export class MemStorage implements IStorage {
  ...
  async createGoal(insertGoal: InsertGoal): Promise<Goal> {
    ...
    const goal: Goal = { ...insertGoal, id, completed: false };
  ...
  async getBudgetByCategory(userId: number, categoryId: number, month: number, year: number): Promise<Budget | undefined> {
    return Array.from(this.budgets.values()).find(
      (budget) => budget.userId === userId && budget.categoryId === categoryId && budget.month === month && budget.year === year
    );
  }
  ...
  private seedData() {
    ...
    const categories = [
      { id: this.currentCategoryId++, name: "Housing", icon: "home", userId: null },
      { id: this.currentCategoryId++, name: "Food", icon: "restaurant", userId: null },
      { id: this.currentCategoryId++, name: "Transportation", icon: "directions_bus", userId: null },
      { id: this.currentCategoryId++, name: "Utilities", icon: "power", userId: null },
      { id: this.currentCategoryId++, name: "Healthcare", icon: "local_hospital", userId: null },
      { id: this.currentCategoryId++, name: "Personal", icon: "person", userId: null },
      { id: this.currentCategoryId++, name: "Education", icon: "school", userId: null },
      { id: this.currentCategoryId++, name: "Other", icon: "more_horiz", userId: null }
    ];
    ...
    const budgets = [
      { id: this.currentBudgetId++, amount: "650", categoryId: 1, userId: user.id, month: currentMonth, year: currentYear },
      { id: this.currentBudgetId++, amount: "300", categoryId: 2, userId: user.id, month: currentMonth, year: currentYear },
      { id: this.currentBudgetId++, amount: "200", categoryId: 3, userId: user.id, month: currentMonth, year: currentYear },
      { id: this.currentBudgetId++, amount: "150", categoryId: 4, userId: user.id, month: currentMonth, year: currentYear },
      { id: this.currentBudgetId++, amount: "100", categoryId: 5, userId: user.id, month: currentMonth, year: currentYear },
      { id: this.currentBudgetId++, amount: "100", categoryId: 6, userId: user.id, month: currentMonth, year: currentYear }
    ];
    ...
    const expenses = [
      { id: this.currentExpenseId++, description: "Grocery Store", amount: "78.25", date: "2023-09-15", categoryId: 2, userId: user.id },
      { id: this.currentExpenseId++, description: "Lunch Cafe", amount: "12.50", date: "2023-09-14", categoryId: 2, userId: user.id },
      { id: this.currentExpenseId++, description: "Public Transit", amount: "5.75", date: "2023-09-13", categoryId: 3, userId: user.id },
      { id: this.currentExpenseId++, description: "Electric Bill", amount: "87.32", date: "2023-09-10", categoryId: 4, userId: user.id },
      { id: this.currentExpenseId++, description: "Rent", amount: "650", date: "2023-09-01", categoryId: 1, userId: user.id }
    ];
    ...
    const articles = [
      { 
        ...
        description: "Learn the essentials of creating and sticking to a personal budget that works for you.", 
        content: `
        ...
        `, 
        imageUrl: "https://images.unsplash.com/photo-1579621970588-a35d0e7ab9b6?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=500&h=300&q=80", 
        category: "budgeting" 
      },
      { 
        id: this.currentArticleId++, 
        title: "Understanding Credit Scores", 
        description: "Demystifying credit scores and how to improve yours over time.", 
        content: `
        ...
        `, 
        imageUrl: "https://images.unsplash.com/photo-1567427017947-545c5f8d16ad?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=500&h=300&q=80", 
        category: "credit" 
      },
      { 
        id: this.currentArticleId++, 
        title: "Saving Strategies", 
        description: "Practical tips for building savings, even when money is tight.", 
        content: `
        ...
        `, 
        imageUrl: "https://images.unsplash.com/photo-1434626881859-194d67b2b86f?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=500&h=300&q=80", 
        category: "saving" 
      }
    ];
